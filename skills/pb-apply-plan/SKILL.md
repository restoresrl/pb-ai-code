---
name: pb-apply-plan
description: Use this when you have a plan file generated by the pb-review flow (under .pb-review/<scope>-<context>-<timestamp>.md) and you want to apply its fixes one at a time to the PowerBuilder workspace. Parses the YAML front-matter of each finding, topo-sorts the queue (inheritance + call-graph + priority), and runs an interactive apply loop with per-fix confirmation and release-note recording after successful imports. Each edit lands through the pb-orca-mcp file loop: pb_object_export_file, edit the file, pb_object_import_file.
metadata:
  version: "3.1.0"
---
