---
description: Run a structured code review on a PowerBuilder target. Frames the work, builds a scoped context pack, validates understanding, writes a plan file + CHANGELOG entry, then hands off to pb-apply-plan for the edit loop.
...
plan file and the CHANGELOG entry → Step 4, hand off to
`pb-apply-plan`.
