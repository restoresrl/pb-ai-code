...
`harness/`, which `pb-orca-mcp` and `pb-format` versions. Two developers
...
- Every human-facing document has had an editing pass. Sentences overloaded
  with long dashes were rewritten, along with run-ons and a few formulaic
  transitions. Code examples and exact output strings were left alone. The
  technical meaning is unchanged.
...
- **`docs/plan-file-contract.md`**: the schema of a `.pb-review` plan file,
  ...
  had been written against each other's prose, and prose drifts quietly. A
  field the consumer ignores raises no error; it just gives the wrong answer
  on the one workspace where it mattered.
...
- **`--pb-version`, and a question when there is a terminal.** Every ORCA call
  needs the version. `pb_session_open` takes it explicitly, with no auto-pick,
  and these machines routinely carry 19, 22 and 25 side by side,
...
v0.1.10 changes review behaviour. This goes beyond wording.
...
- **Neither ORCA size field is the size of the export.** `source_size` is
  ...
  scope has to budget from, is unrelated: 2.7× to 7.9× **over** for
  PowerScript entries and 0.58× to 0.67× **under** for DataWindows. On one
...
- **`pb-appeon-index` refused to run on Python 3.10**, which `requires-python`
  ...
  **Found by CI on its first run, which is the whole argument for having it:**
  no local run could have caught it because this machine uses 3.14. Eight
...
- **A route back for what a project learns.** The knowledge base is
  installed as a snapshot, which keeps every project on one version. An
  agent that discovers something cannot write it down where it works. Two skills already told it to record the discovery under
...
- The installer copies loose documentation files as well as trees. Found by
...
- `tests/test_pins_in_sync.py` checks both full URLs and bare `@vX.Y.Z`
  mentions. The prose sentence explaining what the pin is for had drifted a
...
- `harness/claude-code/settings.json` pre-approves `mcp__pb-appeon-index__*`
  ...
  installer never writes. That is intentional. It remains inert until someone
  adds the server by hand, then saves them a step, but nothing said so.
  Now it does.
...
- **The optional Appeon doc index could not start either**, and for the same
  ...
  module at module scope, so the failure affected every subcommand, including
  the `pb-appeon-index update` that `docs/install.md`
...
- The antipattern catalog's index linked `/pb-review` at
  ...
  install because that harness has no commands directory. It now points at the
  skill, which every layout has.
