...
next install overwrites. That is deliberate. It keeps every project on one
version instead of a slowly diverging copy, but it has a consequence: **an agent that learns something cannot write it down where
it works.**
