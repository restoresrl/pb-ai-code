...
| `--skills-dir REL` | — | required for `generic`, rejected for `claude-code`; target-relative only |
| `--commands-dir REL` | — | optional; must be a sibling of `--skills-dir` |
| `--skip-mcp-config` | off | |
| `--dry-run` | off | |
...
| 3 | — | no marker at this target (documented deviation: agents branch on "installed?" without parsing) |
